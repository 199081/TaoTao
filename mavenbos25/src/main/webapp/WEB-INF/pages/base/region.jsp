...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#button-import").upload({
	        name: 'upload',//file中的name的名字
	        action: '${pageContext.request.contextPath}/region_importData.action',//请求url
	        //enctype: 'multipart/form-data',//类型，默认值，可以省略
	        //params: {},//额外的参数，如果没有可以省略
	        //autoSubmit: true,//默认值，可以省略
	        //onSubmit: function() {},
	        onComplete: function(response) {
	        	//response:ajax的响应内容{"result":true}
	        //参数其实是页面上的一个字符串，响应的字符串"{'result':true}"
	        //alert(response);
	       	 var data =$.parseJSON(response);
	       	 //alert(data);
	        	if(data.result){
	        		//判断是否真的成功
		        	$.messager.alert("信息","导入文件成功","info");	
	        	}else{
	        		$.messager.alert("错误","导入文件失败","error");
	        	}
	        	
	        },
	        onSelect: function() {//选择窗口关闭的时候触发
	        	//先关闭自动提交
	        	...
	        	if(!regexp.test(this.filename())){
	        		$.messager.alert("警告","您上传的文件格式不是excel格式","warning");
	        	}else{
	        		//提交请求
	        		this.submit();
	        		//alert("ok");
