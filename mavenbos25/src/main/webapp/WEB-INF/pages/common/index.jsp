...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#btnEp").click(function(){
			...
			//判断密码是否为空
			//undefined和null的区别
			//undefined：未定义的(未实例化)，没有值的 var aa;
			//null:对象不存在.getElmentById("username1")
			//undefined==null;  undefined===null(错)
			if(txtNewPass==undefined || $.trim(txtNewPass)==""){
				//提示
			...
			$.post("${pageContext.request.contextPath}/user_editpassword.action",{"password":txtNewPass},function(data){
				//data，就返回一个json，用来判断是否修改成功，--如果成功，要关闭修改窗口
				//data={"result",true}
				if(data.result){
					//修改失败
					$.messager.alert("信息","密码修改成功","info");
					//修改成功，关闭修改窗口
					$("#editPwdWindow").window("close");
				}else{
					//修改失败
	...
	function logoutFun() {
		...
		.confirm('系统提示','您确定要退出本次登录吗?',function(isConfirm) {
			if (isConfirm) {
				location.href = '${pageContext.request.contextPath }/user_loginout.action';
			}
		});
	}
	// 修改密码
